...
- [x] Language selector for focus language
- [x] Dashboard shows only focus language when enabled
- [x] Practice sessions only include focus language
- [x] Easy toggle on/off
- [x] Focus language switchable anytime
...
export class FocusModeService {
  ...
  async switchFocusLanguage(userId: string, newLanguage: string): Promise<FocusModeSettings> {
    ...
    await this.pool.query(
      ...
      [userId, newLanguage, JSON.stringify({ from: currentSettings.focusLanguage })]
    );
...
export async function focusModeRoutes(fastify: FastifyInstance) {
  ...
  fastify.get(
    ...
    async (request, reply) => {
      ...
      try {
        const history = await focusModeService.getFocusModeHistory(userId, limit || 30);
